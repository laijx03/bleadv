...
static void detection_delay_timeout_handler(void * p_context)
{
	...
	for (i = 0; i < m_button_count; i++)
	{
		...
		if (btn_mask & m_pin_transition)
		{
			...
			_Bool 
				pin_is_set = nrf_drv_gpiote_in_is_set(p_btn->pin_no);
...
uint32_t app_button_enable(void)
{
	if (0) { if (mp_buttons) { } else { assert_nrf_callback((uint16_t)182, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/button/app_button.c"); } };
	uint32_t i;
	for (i = 0; i < m_button_count; i++)
	{
		nrf_drv_gpiote_in_event_enable(mp_buttons[i].pin_no, 
				1
				);
	}
	return ((0x0) + 0);
...
uint32_t app_button_disable(void)
{
	if (0) { if (mp_buttons) { } else { assert_nrf_callback((uint16_t)196, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/button/app_button.c"); } };
	uint32_t i;
	for (i = 0; i < m_button_count; i++)
	{
...
app_button_is_pushed(uint8_t button_id)
{
	if (0) { if (button_id <= m_button_count) { } else { assert_nrf_callback((uint16_t)211, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/button/app_button.c"); } };
	if (0) { if (mp_buttons != 
			((void *)0)
		    ) { } else { assert_nrf_callback((uint16_t)212, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/button/app_button.c"); } };
	app_button_cfg_t const * p_btn = &mp_buttons[button_id];
	_Bool 
		is_set = nrf_drv_gpiote_in_is_set(p_btn->pin_no);
