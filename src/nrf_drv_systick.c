...
nrf_drv_systick_test(nrf_drv_systick_state_t const * p_state, uint32_t us)
{
	if (0) { if (us <= (((0xFFFFFFUL ) / 2UL) / ((SystemCoreClock) / (1000UL * (1000UL))))) { } else { assert_nrf_callback((uint16_t)139, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/drivers_nrf/systick/nrf_drv_systick.c"); } };;
	const uint32_t diff = (0xFFFFFFUL ) & ((p_state->time) - nrf_systick_val_get());
	return (diff >= nrf_drv_systick_us_tick(us));
}
void nrf_drv_systick_delay_ticks(uint32_t ticks)
{
	if (0) { if (ticks <= ((0xFFFFFFUL ) / 2UL)) { } else { assert_nrf_callback((uint16_t)147, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/drivers_nrf/systick/nrf_drv_systick.c"); } }
...
void nrf_drv_systick_delay_us(uint32_t us)
{
	if (0) { if (us <= (((0xFFFFFFUL ) / 2UL) / ((SystemCoreClock) / (1000UL * (1000UL))))) { } else { assert_nrf_callback((uint16_t)158, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/drivers_nrf/systick/nrf_drv_systick.c"); } };;
...
void nrf_drv_systick_delay_ms(uint32_t ms)
{
	...
	while (0 != (n--))
	{
		nrf_drv_systick_delay_ticks(nrf_drv_systick_ms_tick((64U)));
	}
