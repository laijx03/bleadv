...
static struct {
	...
	u8_t data[(1 + 76)] 
		...
		;
	u8_t ud[(((unsigned long)(sizeof(struct cmd_data)) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1))] 
		__attribute__((__aligned__(
						sizeof(int)
					  )))
		;
} _net_buf_hci_cmd_pool_name[2];
static struct {
	struct net_buf buf;
	u8_t data[(1 + 76)] 
		__attribute__((__aligned__(
		...
		;
	u8_t ud[(((unsigned long)(4) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1))] 
		__attribute__((__aligned__(
						sizeof(int)
					  )))
		;
} _net_buf_hci_rx_pool_name[10];
static struct {
	struct net_buf buf;
	u8_t data[(1 + 4 + 4 + (65))] 
		__attribute__((__aligned__(
	...
	u8_t ud[(((unsigned long)(4) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1))] 
		...
		;
} _net_buf_acl_tx_pool_name[10];
struct net_buf_pool hci_cmd_pool 
	__attribute__((__aligned__(
					sizeof(int)
				  ))) 
	= { .free = { ._queue = { 0 } }, .__bufs = (struct net_buf *)_net_buf_hci_cmd_pool_name, .buf_count = 2, .uninit_count = 2, .buf_size = (1 + 76), .user_data_size = sizeof(struct cmd_data), .destroy = 
		((void *)0)
			, }
			;
struct net_buf_pool hci_rx_pool 
	...
	= { .free = { ._queue = { 0 } }, .__bufs = (struct net_buf *)_net_buf_hci_rx_pool_name, .buf_count = 10, .uninit_count = 10, .buf_size = (1 + 76), .user_data_size = 4, .destroy = 
		((void *)0)
			, }
			;
struct net_buf_pool acl_tx_pool 
	...
	= { .free = { ._queue = { 0 } }, .__bufs = (struct net_buf *)_net_buf_acl_tx_pool_name, .buf_count = 10, .uninit_count = 10, .buf_size = (1 + 4 + 4 + (65)), .user_data_size = 4, .destroy = 
		((void *)0)
			, }
			;
...
static int pool_id(struct net_buf_pool *pool)
{
	...
	((
	  index < (sizeof(net_buf_pool_list) / 4)
	 ) ? (void)0 : __assert_func ("kernel/protocols/bluetooth/common/buf.c", 117, __func__, 
		 "index < (sizeof(net_buf_pool_list) / 4)"
		 ))
		;
	return index;
...
int net_buf_id(struct net_buf *buf)
{
	...
	return (buf_ptr - pool_start) / (sizeof(struct net_buf) + (((unsigned long)(pool->buf_size) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1)) + (((unsigned long)(pool->user_data_size) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1)));
}
static inline struct net_buf *pool_get_uninit(struct net_buf_pool *pool,
		u16_t uninit_count)
{
	struct net_buf *buf;
	buf = (struct net_buf *)(((u8_t *)(pool->__bufs)) + ((pool->buf_count - uninit_count) * (sizeof(struct net_buf) + (((unsigned long)(pool->buf_size) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1)) + (((unsigned long)(pool->user_data_size) + ((unsigned long)4 - 1)) & ~((unsigned long)4 - 1)))));
...
struct net_buf *net_buf_clone(struct net_buf *buf, s32_t timeout)
{
	...
	net_buf_reserve(clone, net_buf_simple_headroom(&(buf)->b));
	memcpy(net_buf_simple_add(&(clone)->b, buf->len), buf->data, buf->len);
	return clone;
}
