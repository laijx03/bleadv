...
static u8_t ticker_by_slot_get(struct ticker_node *node, u8_t ticker_id_head,
		u32_t ticks_slot)
{
	while (ticker_id_head != ((u8_t)((u8_t)0 - 1))) {
		struct ticker_node *ticker;
		u32_t ticks_to_expire;
		ticker = &node[ticker_id_head];
		ticks_to_expire = ticker->ticks_to_expire;
		if (ticks_slot <= ticks_to_expire) {
			return ((u8_t)((u8_t)0 - 1));
		}
		...
		u32_t *ticks_to_expire)
{
	...
	if ((_ticker_id_head == ((u8_t)((u8_t)0 - 1))) ||
	...
	while ((_ticker_id_head != ((u8_t)((u8_t)0 - 1))) &&
...
static u8_t ticker_enqueue(struct ticker_instance *instance, u8_t id)
{
	...
	collide = ticker_id_slot_previous = ((u8_t)((u8_t)0 - 1));
	current = instance->ticker_id_head;
	previous = current;
	ticks_slot_previous = instance->ticks_slot_previous;
	while ((current != ((u8_t)((u8_t)0 - 1))) &&
			(ticks_to_expire >
...
static u32_t ticker_dequeue(struct ticker_instance *instance, u8_t id)
{
	...
	if (ticker_current->next != ((u8_t)((u8_t)0 - 1))) {
...
static inline void ticker_worker(struct ticker_instance *instance)
{
	...
	if (instance->ticker_id_head == ((u8_t)((u8_t)0 - 1))) {
		...
		u32_t ticks_current, u32_t ticks_at_start)
{
	...
	if (((ticks_at_start - ticks_current) & (1UL << (23))) == 0) {
		...
		u8_t *insert_head)
{
	...
	if (user_op->op == TICKER_USER_OP_TYPE_UPDATE) {
		...
		if (instance->ticker_id_slot_previous == user_op->id) {
			instance->ticker_id_slot_previous = ((u8_t)((u8_t)0 - 1));
		...
		u8_t *insert_head)
{
	...
	node = &instance->node[0];
	ticks_expired = 0;
	while (instance->ticker_id_head != ((u8_t)((u8_t)0 - 1))) {
		struct ticker_node *ticker;
		u32_t ticks_to_expire;
		u8_t id_expired;
		...
		u8_t insert_head)
{
	...
	while (count_user--) {
		...
		while ((insert_head != ((u8_t)((u8_t)0 - 1))) ||
				(user_ops_first != user->middle)) {
			struct ticker_user_op *user_op;
			struct ticker_node *ticker;
			u8_t id_insert;
			u32_t status;
			if (insert_head != ((u8_t)((u8_t)0 - 1))) {
		...
		u8_t ticker_id_old_head)
{
	...
	if (ticker_id_old_head == ((u8_t)((u8_t)0 - 1))) {
		u32_t ticks_current;
	...
	do {
		...
		ctr_post = cntr_cnt_get();
				...
				3) > ticker_ticks_diff_get(cc, ctr));
}
		...
		u8_t count_user, void *user, u8_t count_op, void *user_op)
{
	...
	struct ticker_user *users;
	if ((sizeof(struct ticker_node) != 36) ||
			(sizeof(struct ticker_user) != 8) ||
			(sizeof(struct ticker_user_op) != 44)) {
		return 1;
	}
	...
	instance->ticker_id_head = ((u8_t)((u8_t)0 - 1));
	instance->ticker_id_slot_previous = ((u8_t)((u8_t)0 - 1));
	instance->ticks_slot_previous = 0;
...
void ticker_trigger(u8_t instance_index)
{
	...
	if (_instance[instance_index].fp_sched) {
		_instance[instance_index].fp_sched(0,
				1, 1);
	}
	;
}
		...
		ticker_op_func fp_op_func, void *op_context)
{
	...
	user->last = last;
	instance->fp_sched(instance->fp_caller_id_get(user_id), 2, 0);
