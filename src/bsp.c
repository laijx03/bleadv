	...
	if (button < 4)
	{
		switch (button_action)
		{
			case 1:
				...
				{
					err_code = app_timer_start(m_button_timer_id, ((uint32_t)(((((1000)) * (uint64_t)32768) + ((1000 * (0 + 1)) / 2)) / (1000 * (0 + 1)))), (void*)&current_long_push_pin_no);
					if (err_code == ((0x0) + 0))
					{
...
static uint32_t bsp_led_indication(bsp_indication_t indicate)
{
	...
	switch (indicate)
	{
		...
		case BSP_INDICATE_ADVERTISING:
			...
			m_stable_state = indicate;
			err_code = app_timer_start(m_leds_timer_id, ((uint32_t)((((next_delay) * (uint64_t)32768) + ((1000 * (0 + 1)) / 2)) / (1000 * (0 + 1)))), 
					((void *)0)
					);
			break;
		...
		case BSP_INDICATE_BONDING:
			...
			m_stable_state = indicate;
			err_code =
				app_timer_start(m_leds_timer_id, ((uint32_t)((((100) * (uint64_t)32768) + ((1000 * (0 + 1)) / 2)) / (1000 * (0 + 1)))), 
						((void *)0)
					       );
			break;
		...
		case BSP_INDICATE_SENT_OK:
			...
			err_code = app_timer_start(m_leds_timer_id, ((uint32_t)((((100) * (uint64_t)32768) + ((1000 * (0 + 1)) / 2)) / (1000 * (0 + 1)))), 
					((void *)0)
					);
			break;
		case BSP_INDICATE_SEND_ERROR:
			m_leds_clear = 
				...
				;
			bsp_board_led_invert(1);
			err_code = app_timer_start(m_leds_timer_id, ((uint32_t)((((500) * (uint64_t)32768) + ((1000 * (0 + 1)) / 2)) / (1000 * (0 + 1)))), 
					((void *)0)
					);
			break;
