...
static u8_t __attribute__((aligned(4))) rand_context[4 + 4 + 1];
static u8_t __attribute__((aligned(4))) rand_isr_context[4 + 12 + 1];
static u8_t __attribute__((aligned(4))) _ticker_nodes[((7 + 1 ) + 0)][36];
static u8_t __attribute__((aligned(4))) _ticker_users[4][8];
static u8_t __attribute__((aligned(4))) _ticker_user_ops[(((7 + 1) + (2 + 1) + (1 + 1) ) + 0)][44];
	static u8_t __attribute__((aligned(4))) _radio[((sizeof(struct connection) * 1) + (sizeof(void *) * (1 + 4)) + ((sizeof(struct pdu_data_q_tx) * ((1 + (1) ) + 2)) * 2) + ((((u32_t)(
							__builtin_offsetof (
								struct radio_pdu_node_rx
								, 
								pdu_data
							...
							+ (( ((
											...
											+ 27) : ((
													...
													+ 37) + 1)))+3) & (~((u32_t)3))) * (1 + 3)) + (sizeof(void *) * 2 * ((1 + 4) + 1)) + ((((u32_t)(
											__builtin_offsetof (
												struct radio_pdu_node_tx
												, 
												pdu_data
...
void mayfly_enable_cb(u8_t caller_id, u8_t callee_id, u8_t enable)
{
	...
	if (!(callee_id == 1)) { ; ; };
	if (enable) {
		_arch_irq_enable(SWI4_EGU4_IRQn);
	} else {
		_arch_irq_disable(SWI4_EGU4_IRQn);
	}
}
u32_t mayfly_is_enabled(u8_t caller_id, u8_t callee_id)
{
	(void)caller_id;
	if (callee_id == 0) {
		return _arch_irq_is_enabled(RTC0_IRQn);
	} else if (callee_id == 1) {
		return _arch_irq_is_enabled(SWI4_EGU4_IRQn);
	}
	if (!(0)) { ; ; };
	return 0;
}
...
void mayfly_pend(u8_t caller_id, u8_t callee_id)
{
	...
	switch (callee_id) {
		case 0:
			...
			break;
		case 1:
			NVIC_SetPendingIRQ(SWI4_EGU4_IRQn);
			break;
		case 3:
		default:
			if (!(0)) { ; ; };
...
void RTC0_IRQHandler(void *arg)
{
	...
	krhino_intrpt_enter();
	compare0 = ((NRF_RTC_Type *) 0x4000B000UL)->EVENTS_COMPARE[0];
	compare1 = ((NRF_RTC_Type *) 0x4000B000UL)->EVENTS_COMPARE[1];
	if (compare0) {
		((NRF_RTC_Type *) 0x4000B000UL)->EVENTS_COMPARE[0] = 0;
		ticker_trigger(0);
	}
	if (compare1) {
		((NRF_RTC_Type *) 0x4000B000UL)->EVENTS_COMPARE[1] = 0;
		ticker_trigger(1);
	}
	mayfly_run(0);
	krhino_intrpt_exit();
...
int ll_init(struct k_sem *sem_rx)
{
	...
	rand_init(rand_context, sizeof(rand_context), 4);
	rand_isr_init(rand_isr_context, sizeof(rand_isr_context),
			12);
	clk_k32 = device_get_binding("nrf5k32srcclockcontrol");
	if (!clk_k32) {
		return -19;
	}
	...
	cntr_init();
	mayfly_init();
	_ticker_users[0][0] = (7 + 1);
	_ticker_users[1][0] = (2 + 1);
	_ticker_users[2][0] = 0;
	_ticker_users[3][0] = ((1 + 1) + 0);
	ticker_init(0, ((7 + 1 ) + 0),
			&_ticker_nodes[0], 4, &_ticker_users[0],
			(((7 + 1) + (2 + 1) + (1 + 1) ) + 0), &_ticker_user_ops[0]);
	clk_m16 = device_get_binding("nrf5m16srcclockcontrol");
	if (!clk_m16) {
		return -19;
	}
	err = radio_init(clk_m16, 1,
			1,
			1,
			(1 + (1) ),
			27,
			27, &_radio[0], sizeof(_radio));
	if (err) {
		;
		return -12;
	}
	...
	({ static struct _isr_list __attribute__((section(".intList"))) __attribute__((__used__)) __isr_RADIO_IRQHandler_irq___COUNTER__ = {1, (1 << 0), &RADIO_IRQHandler, (void *)
	 ((void *)0)
	 }; os_hwi_set_handler(1, RADIO_IRQHandler, 
			 ((void *)0)
			 ); _irq_priority_set(1, 0, 0); 1; })
		;
	({ static struct _isr_list __attribute__((section(".intList"))) __attribute__((__used__)) __isr_RTC0_IRQHandler_irq___COUNTER__ = {11, 0, &RTC0_IRQHandler, (void *)
	 ((void *)0)
	 }; os_hwi_set_handler(11, RTC0_IRQHandler, 
			 ((void *)0)
			 ); _irq_priority_set(11, 0, 0); 11; })
		;
	({ static struct _isr_list __attribute__((section(".intList"))) __attribute__((__used__)) __isr_SWI4_EGU4_IRQHandler_irq___COUNTER__ = {24, 0, &SWI4_EGU4_IRQHandler, (void *)
	 ((void *)0)
	 }; os_hwi_set_handler(24, SWI4_EGU4_IRQHandler, 
			 ((void *)0)
			 ); _irq_priority_set(24, 0, 0); 24; })
		;
	({ static struct _isr_list __attribute__((section(".intList"))) __attribute__((__used__)) __isr_RNG_IRQHandler_irq___COUNTER__ = {13, 0, &RNG_IRQHandler, (void *)
	 ((void *)0)
	 }; os_hwi_set_handler(13, RNG_IRQHandler, 
			 ((void *)0)
			 ); _irq_priority_set(13, 1, 0); 13; });
	_arch_irq_enable(1);
	_arch_irq_enable(11);
	_arch_irq_enable(24);
	_arch_irq_enable(13);
...
u8_t *ll_addr_get(u8_t addr_type, u8_t *bdaddr)
{
	...
	if (addr_type) {
		if (bdaddr) {
			memcpy(bdaddr, _ll_context.rnd_addr, 6);
		}
		return _ll_context.rnd_addr;
	}
	if (bdaddr) {
		memcpy(bdaddr, _ll_context.pub_addr, 6);
	}
...
{
	if (addr_type) {
		memcpy(_ll_context.rnd_addr, bdaddr, 6);
	} else {
		memcpy(_ll_context.pub_addr, bdaddr, 6);
	}
}
