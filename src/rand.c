...
static void init(struct rand **rng, u8_t *context, u8_t len, u8_t threshold)
{
	...
	*rng = (struct rand *)context;
	...
	if (!rng_isr || !rng_thr) {
		...
		((NRF_RNG_Type *) 0x4000D000UL)->INTENSET = (0x1UL << (0UL));
...
static int isr(struct rand *rng, 
		_Bool 
		store)
{
	u8_t last;
	...
	rng->rand[rng->last] = ((NRF_RNG_Type *) 0x4000D000UL)->VALUE;
	rng->last = last;
	last = rng->last + 1;
	if (last == rng->count) {
		last = 0;
...
{
	(void)(param);
	if (((NRF_RNG_Type *) 0x4000D000UL)->EVENTS_VALRDY) {
		int ret;
		ret = isr(rng_isr, 
				1
			 );
		if (ret != -16) {
			ret = isr(rng_thr, (ret == -55));
		}
		((NRF_RNG_Type *) 0x4000D000UL)->EVENTS_VALRDY = 0;
		if (ret != -16) {
			((NRF_RNG_Type *) 0x4000D000UL)->TASKS_STOP = 1;
		}
	}
}
