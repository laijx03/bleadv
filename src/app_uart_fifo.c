		...
		app_irq_priority_t irq_priority)
{
	...
	m_rx_ovf = 
		0
		;
	if (p_comm_params->rx_pin_no != 0xFFFFFFFF)
	{
...
uint32_t app_uart_get(uint8_t * p_byte)
{
	if (0) { if (p_byte) { } else { assert_nrf_callback((uint16_t)191, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/uart/app_uart_fifo.c"); } };
	_Bool 
		rx_ovf = m_rx_ovf;
	...
	if (rx_ovf)
	{
		...
		do { const uint32_t LOCAL_ERR_CODE = (uart_err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler_bare((LOCAL_ERR_CODE)); } while (0); } } while (0);
	}
	return err_code;
}
uint32_t app_uart_put(uint8_t byte)
...
int32_t hal_uart_recv(uart_dev_t *uart, void *data, uint32_t expect_size, uint32_t timeout)
{
	...
	{
		ret = krhino_buf_queue_recv(&g_buf_queue_uart, ((uint64_t)-1), &pdata[i], &rev_size);
		if((ret == 0) && (rev_size == 1))
		{
			rx_count++;
		}else {
