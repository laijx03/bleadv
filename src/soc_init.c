...
int32_t hal_uart_init(uart_dev_t* uart )
{
	...
	do { app_uart_buffers_t buffers; static uint8_t rx_buf[128]; static uint8_t tx_buf[512]; buffers.rx_buf = rx_buf; buffers.rx_buf_size = sizeof (rx_buf); buffers.tx_buf = tx_buf; buffers.tx_buf_size = sizeof (tx_buf); err_code = app_uart_init(&comm_params, &buffers, uart_handler, APP_IRQ_PRIORITY_LOWEST); } while (0)
		;
...
int __io_getchar(void)
{
	uint8_t ch = 
		...
		;
	int32_t ret = -1;
	ret = hal_uart_recv(&uart_0, &ch, 1, 0xffffffffU);
	if (ret == 0) {
		return ch;
	} else {
		return -1;
...
inline void cpu_hardfault_handle(void)
{
	__asm volatile("MRS %0,""msp""\r\n":"=r"(g_msp_value));
	__asm volatile("MRS %0,""psp""\r\n":"=r"(g_psp_value));
