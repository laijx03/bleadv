...
void ble_sample(void)
{
	...
	const struct bt_data adv_data[] = {
		{ .type = (0x01), .data_len = (1), .data = (const u8_t *)(data), },
		{ .type = (0x09), .data_len = (strlen(adv_name)), .data = (const u8_t *)(adv_name), }
	};
	...
	do {
		aos_msleep(400);
		err = bt_le_adv_start(&adv_param, adv_data, ((unsigned long) (((int) sizeof(char[1 - 2 * !(!__builtin_types_compatible_p(__typeof__(adv_data), __typeof__(&(adv_data)[0])))]) - 1) + (sizeof(adv_data) / sizeof((adv_data)[0])))), 
				((void *)0)
				, 0);
		if (err) {
...
{
	aos_post_delayed_action(1000, app_delayed_action, 
			((void *)0)
			);
	aos_loop_run();
	return 0;
