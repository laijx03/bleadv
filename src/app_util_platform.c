...
void app_util_critical_region_enter(uint8_t *p_nested)
{
	if (0) { if (APP_LEVEL_PRIVILEGED == privilege_level_get()) { } else { assert_nrf_callback((uint16_t)67, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/util/app_util_platform.c"); } }
	app_util_disable_irq();
}
void app_util_critical_region_exit(uint8_t nested)
{
	if (0) { if (APP_LEVEL_PRIVILEGED == privilege_level_get()) { } else { assert_nrf_callback((uint16_t)81, (uint8_t *)"platform/mcu/nrf52xxx/Drivers/libraries/util/app_util_platform.c"); } }
	app_util_enable_irq();
}
uint8_t privilege_level_get(void)
{
