...
_Bool 
ais_ota_check_if_resume(uint8_t * p_data, uint16_t length);
...
{
	if (evt->id == NRF_FSTORAGE_EVT_ERASE_RESULT && evt->result == ((0x0) + 0)) {
		if (flash_handler) flash_handler(ALI_OTA_FLASH_ERASE_OK);
	} else if (evt->id == NRF_FSTORAGE_EVT_WRITE_RESULT && evt->result == ((0x0) + 0)) {
...
ali_ota_flash_err_t ais_ota_flash_erase(uint32_t const *addr, uint32_t num_pages, flash_event_handler_t cb)
{
	...
	return err_code == ((0x0) + 0) ? ALI_OTA_FLASH_CODE_SUCCESS : ALI_OTA_FLASH_CODE_ERROR;
}
ali_ota_flash_err_t ais_ota_flash_store(uint32_t const *addr, uint32_t const * p_data, uint16_t len, flash_event_handler_t cb)
{
	uint32_t err_code;
...
uint32_t ais_ota_get_dst_addr()
{
	return 0x10000 + align_to_page(s_dfu_settings.bank_0.image_size, ais_ota_get_page_size()) + 0x1000;
}
static void bootloader_settings_event_handler_helper(nrf_fstorage_evt_t const * const evt)
{
	if (evt->id == NRF_FSTORAGE_EVT_WRITE_RESULT && evt->result == ((0x0) + 0)) {
...
{
	ret_code_t err_code;
	do { if (1) { csp_printf("[%06d]<" "V" "> ", (unsigned)aos_now_ms(), "Writing settings..."); csp_printf("Writing settings..."); csp_printf("\r\n"); } } while (0);
	err_code = nrf_dfu_flash_erase((uint32_t)0xff000, 1, 
			((void *)0)
			);
	if (err_code != ((0x0) + 0))
	{
		do { if (1) { csp_printf("[%06d]<" "V" "> ", (unsigned)aos_now_ms(), "Could not erase the settings page!"); csp_printf("Could not erase the settings page!"); csp_printf("\r\n"); } } while (0);
		return ((0x0) + 3);
	}
	...
	err_code = nrf_dfu_flash_store((uint32_t)0xff000,
			&temp_dfu_settings,
	...
	if (err_code != ((0x0) + 0))
	{
		do { if (1) { csp_printf("[%06d]<" "V" "> ", (unsigned)aos_now_ms(), "Could not write the DFU settings page!"); csp_printf("Could not write the DFU settings page!"); csp_printf("\r\n"); } } while (0);
		return ((0x0) + 3);
	}
	return ((0x0) + 0);
}
ali_ota_settings_err_t ais_ota_settings_write(settings_event_handler_t cb)
{
	...
	return err_code == ((0x0) + 0) ? ALI_OTA_SETTINGS_CODE_SUCCESS : ALI_OTA_SETTINGS_CODE_ERROR;
}
	_Bool 
ais_ota_check_if_resume(uint8_t * p_data, uint16_t length)
{
...
ais_ota_check_if_update_finished()
{
	return s_dfu_settings.init_command[0] == 0xAB;
}
void ais_ota_update_settings_after_update_finished()
{
	s_dfu_settings.init_command[0] = 0xFF;
	memset(s_dfu_settings.init_command+1, 0xFF, 8 + sizeof(uint16_t) + sizeof(uint32_t) + 1);
	s_dfu_settings.progress.firmware_image_offset = 0;
}
void ais_ota_update_setting_after_xfer_finished(uint32_t img_size, uint32_t img_crc)
{
	{
		s_dfu_settings.bank_current = 1;
		s_dfu_settings.bank_1.image_size = img_size;
		s_dfu_settings.bank_1.bank_code = 0x01;
...
ais_ota_check_if_bins_supported()
{
	return s_dfu_settings.bins_flag == 1 ? 
		1 
		: 
		0
		;
